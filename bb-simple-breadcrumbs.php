<?php
/**
 * Plugin Name: BB Simple Breadcrumbs
 * Plugin URI: https://trisummitsolutions.com
 * Description: Custom breadcrumbs module for the Beaver Builder Plugin.
 * Version: 1.0
 * Author: Morgan Tuttle
 * Author URI: https://trisummitsolutions.com
 */
define( 'BB_SIMPLE_BREADCRUMBS_DIR', plugin_dir_path( __FILE__ ) );
define( 'BB_SIMPLE_BREADCRUMBS_DIR', plugins_url( '/', __FILE__ ) );

function my_load_module_breadcrumbs() {
    if ( class_exists( 'FLBuilder' ) ) {
        require_once 'tss-breadcrumbs/tss-breadcrumbs.php';
    }
}
...
if ( ! function_exists( 'tss_get_breadcrumbs' ) ) {
	function tss_get_breadcrumbs( $args = array() ) {
		if ( is_front_page() ) {
			return;
		}
		if ( get_theme_mod( 'tss_show_breadcrumbs_setting' ) == 'no' ) {
			return;
		}
		global $post;
		$defaults  = array(
		...
		$args      = apply_filters( 'tss_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
		...
		$html .= '<span class="item-home"><a class="bread-link bread-home bread-parent" href="' . get_home_url() . '" title="' . esc_attr( $args['home_title'] ) . '">' . esc_html( $args['home_title'] ) . '</a></span>';
		$html .= $separator;
		// Post
		if ( is_singular( 'post' ) ) {
		...
		$html .= '</div>';
		$html = apply_filters( 'tss_breadcrumbs_filter', $html );
